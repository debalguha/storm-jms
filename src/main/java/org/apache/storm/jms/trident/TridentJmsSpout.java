...
public class TridentJmsSpout implements ITridentSpout<JmsBatch> {
	 ...
	 * 
	 * @param name
	 *            The name to be used in log messages
	 * @return This spout
	 */
	...
	/**
	 * Set the <code>JmsProvider</code> implementation that this Spout will use
	 * to connect to a JMS <code>javax.jms.Desination</code>
	 * 
	 * @param provider
	 */
	 ...
	 * </ul>
	 * 
	 * @param jmsAcknowledgeMode
	 *            The chosen acknowledge mode
	 * @return This spout
	...
	/**
	 * Return a friendly string for the given JMS acknowledge mode, or throw an
	...
	@Override
	public BatchCoordinator<JmsBatch> getCoordinator(String txStateId, @SuppressWarnings("rawtypes") Map conf, TopologyContext context) {
	 ...
	 * blocking queue. On each invocation of emit, the queued messages are
	 * emitted as a batch.
	 *
	 */
	private class JmsEmitter implements Emitter<JmsBatch>, MessageListener {
		...
		private final RotatingMap<Long, List<Message>> batchMessageMap; // Maps
																		// transaction
																		// Ids
																		// to
																		// JMS
		 ...
		 * batch is timed out, or a new batch with a matching transaction id is
		 * emitted. Note that the current implementation does nothing - i.e. it
		 * discards messages that have been failed.
		 * 
		 * @param transactionId
		 *            The transaction id of the failed batch
		 * @param messages
		 *            The list of messages to fail.
		 */
